...
# frequency_list = ['40Hz', '20Hz', '40Hz']


output_dir = 'Stimulation_2nd_level_OutputDir_40Hz'
working_dir = 'Stimulation_2nd_level_WorkingDir_40Hz'

stimulation_2nd_level = Workflow(name='stimulation_2nd_level_40Hz')
...
templates = {
    ...
    'func_2_anat_trans_40Hz_r1':  """/media/amr/Amr_4TB/Work/stimulation/Stimulation_Preproc_WorkingDir/stimulation_preproc/_frequency_id_40Hz_session_id_run001_subject_id_{subject_id}/coreg/bold_2_anat_sub-{subject_id}0GenericAffine.mat""",
    'func_2_anat_trans_40Hz_r2':  """/media/amr/Amr_4TB/Work/stimulation/Stimulation_Preproc_WorkingDir/stimulation_preproc/_frequency_id_40Hz_session_id_run002_subject_id_{subject_id}/coreg/bold_2_anat_sub-{subject_id}0GenericAffine.mat""",

    # ==========================================================================================================================================================

    'cope1_40Hz_r1': '/media/amr/Amr_4TB/Work/stimulation/Stimulation_1st_level_OutputDir/copes_1st_level/40Hz_run001_subj_{subject_id}/cope1.nii.gz',
    'cope1_40Hz_r2': '/media/amr/Amr_4TB/Work/stimulation/Stimulation_1st_level_OutputDir/copes_1st_level/40Hz_run002_subj_{subject_id}/cope1.nii.gz',

    # ==========================================================================================================================================================

    'varcope1_40Hz_r1': '/media/amr/Amr_4TB/Work/stimulation/Stimulation_1st_level_OutputDir/varcopes_1st_level/40Hz_run001_subj_{subject_id}/varcope1.nii.gz',
    'varcope1_40Hz_r2': '/media/amr/Amr_4TB/Work/stimulation/Stimulation_1st_level_OutputDir/varcopes_1st_level/40Hz_run002_subj_{subject_id}/varcope1.nii.gz',
    # ==========================================================================================================================================================

    "percent_change_r1": '/media/amr/Amr_4TB/Work/stimulation/Stimulation_1st_level_WorkingDir_%_change/stimulation_1st_level_percent_change/_frequency_id_40Hz_session_id_run001_subject_id_{subject_id}/get_percent_change_timeseries/percent_change_timeseries.txt',
    "percent_change_r2": '/media/amr/Amr_4TB/Work/stimulation/Stimulation_1st_level_WorkingDir_%_change/stimulation_1st_level_percent_change/_frequency_id_40Hz_session_id_run002_subject_id_{subject_id}/get_percent_change_timeseries/percent_change_timeseries.txt',
...
def copes1_2_anat_func(fixed, cope1_40Hz_r1, cope1_40Hz_r2, func_2_anat_trans_40Hz_r1, func_2_anat_trans_40Hz_r2, mask_brain):
    import os
    import re
    import nipype.interfaces.ants as ants
    import nipype.interfaces.fsl as fsl
    ...
    copes1 = [cope1_40Hz_r1, cope1_40Hz_r2]
    trans = [func_2_anat_trans_40Hz_r1, func_2_anat_trans_40Hz_r2]
    ...
    for i in range(len(copes1)):
        ...
        ants_apply.inputs.output_image = 'cope1_2_anat_40Hz_r{0}.nii.gz'.format(i+1)

        ants_apply.run()

        copes1_2_anat.append(os.path.abspath('cope1_2_anat_40Hz_r{0}.nii.gz'.format(i+1)))

        dof = fsl.ImageMaths()
        dof.inputs.in_file = 'cope1_2_anat_40Hz_r{0}.nii.gz'.format(i+1)
        dof.inputs.op_string = '-mul 0 -add 147 -mas'
        dof.inputs.in_file2 = mask_brain
        dof.inputs.out_file = 'FEtdof_t1_2_anat_40Hz_r{0}.nii.gz'.format(i+1)

        dof.run()

        FEtdof_t1_2_anat.append(os.path.abspath('FEtdof_t1_2_anat_40Hz_r{0}.nii.gz'.format(i+1)))

    merge = fsl.Merge()
    merge.inputs.dimension = 't'
    merge.inputs.in_files = copes1_2_anat
    merge.inputs.merged_file = 'copes1_2_anat_40Hz.nii.gz'
    merge.run()

    merge.inputs.in_files = FEtdof_t1_2_anat
    merge.inputs.merged_file = 'dofs_t1_2_anat_40Hz.nii.gz'
    merge.run()

    copes1_2_anat = os.path.abspath('copes1_2_anat_40Hz.nii.gz')
    dofs_t1_2_anat = os.path.abspath('dofs_t1_2_anat_40Hz.nii.gz')
...
copes1_2_anat_func = Node(name='copes1_2_anat_func',
                          interface=Function(input_names=['fixed',
                                                          'cope1_40Hz_r1',
                                                          'cope1_40Hz_r2',
                                                          'func_2_anat_trans_40Hz_r1',
                                                          'func_2_anat_trans_40Hz_r2',
                                                          'mask_brain'],
...
def varcopes1_2_anat_func(fixed, varcope1_40Hz_r1, varcope1_40Hz_r2, func_2_anat_trans_40Hz_r1, func_2_anat_trans_40Hz_r2):
    import os
    import re
    import nipype.interfaces.ants as ants
    import nipype.interfaces.fsl as fsl
    ...
    varcopes1 = [varcope1_40Hz_r1, varcope1_40Hz_r2]
    trans = [func_2_anat_trans_40Hz_r1, func_2_anat_trans_40Hz_r2]
    ...
    for i in range(len(varcopes1)):
        ...
        ants_apply.inputs.reference_image = fixed
        ants_apply.inputs.transforms = transform
        ants_apply.inputs.output_image = 'varcope1_2_anat_40Hz_r{0}.nii.gz'.format(i+1)

        ants_apply.run()

        varcopes1_2_anat.append(os.path.abspath('varcope1_2_anat_40Hz_r{0}.nii.gz'.format(i+1)))

    merge = fsl.Merge()
    merge.inputs.dimension = 't'
    merge.inputs.in_files = varcopes1_2_anat
    merge.inputs.merged_file = 'varcopes1_2_anat_40Hz.nii.gz'
    merge.run()

    varcopes1_2_anat = os.path.abspath('varcopes1_2_anat_40Hz.nii.gz')
...
# t constrast does not produce any activation, that's why i am using f contrast

design = '/media/amr/Amr_4TB/Work/stimulation/1st_Level_Designs/design_2nd_level_2_sessions.mat'
t_contrast = '/media/amr/Amr_4TB/Work/stimulation/1st_Level_Designs/design_2nd_level_2_sessions.con'
f_contrast = '/media/amr/Amr_4TB/Work/stimulation/1st_Level_Designs/design_2nd_level_2_sessions.fts'
grp_contrast = '/media/amr/Amr_4TB/Work/stimulation/1st_Level_Designs/design_2nd_level_2_sessions.grp'
...
stimulation_2nd_level.connect([
    ...
    (selectfiles, copes1_2_anat_func, [('anat_brain', 'fixed'),
                                       ('cope1_40Hz_r1', 'cope1_40Hz_r1'),
                                       ('cope1_40Hz_r2', 'cope1_40Hz_r2'),
                                       ('func_2_anat_trans_40Hz_r1',
                                        'func_2_anat_trans_40Hz_r1'),
                                       ('func_2_anat_trans_40Hz_r2',
                                        'func_2_anat_trans_40Hz_r2'),
    ...
    (selectfiles, varcopes1_2_anat_func, [('anat_brain', 'fixed'),
                                          ('varcope1_40Hz_r1', 'varcope1_40Hz_r1'),
                                          ('varcope1_40Hz_r2', 'varcope1_40Hz_r2'),
                                          ('func_2_anat_trans_40Hz_r1', 'func_2_anat_trans_40Hz_r1'),
                                          ('func_2_anat_trans_40Hz_r2', 'func_2_anat_trans_40Hz_r2')]),
